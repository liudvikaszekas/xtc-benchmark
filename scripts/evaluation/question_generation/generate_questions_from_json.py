...
def _trace_meta(obj: dict, obj_idx: int) -> dict:
    ...
    Prefer seg_id (instance-level). Fallback to key index (box_id or original_index).
    """
    return {
...
def generate_required_vqa_questions(
        ...
        for attr_key, raw_val in sorted(attrs.items(), key=lambda kv: str(kv[0])):
            ...
            out.append({
                ...
                "reference": {
                    ...
                    "object_id": obj.get("original_index") or obj.get("box_id"),
                ...
                if use_multiple_choice and all_predicates:
                    ...
                    out.append({
                        ...
                        "reference": {
                            "type": "relationship",
                            "scene_graph": scene_graph_id,
                            "subject_id": objects[subj_idx].get("original_index") or objects[subj_idx].get("box_id"),
                            "object_id": objects[obj_idx].get("original_index") or objects[obj_idx].get("box_id"),
                            "predicates": all_rels
                        },
                        "meta": {
    ...
    # --- Type D: Counting (How many <label>s?) ---
    pass
...
def load_graph_merge_scenes(dir_path: str):
    """Load scenes from graph_merge step output directory.
    
    Reads per-image scene-graph_*.json files from the graph_merge step.
    Since groups of 3 or more have already been processed in this step,
    relations here correctly reflect the merged state without needing manual filtering.
    """
    if not os.path.isdir(dir_path):
        raise ValueError(f"Graph merge directory does not exist: {dir_path}")
    
    scene_files = sorted(glob.glob(os.path.join(dir_path, "scene-graph_*.json")))
    if not scene_files:
        raise FileNotFoundError(
            f"No scene-graph_*.json files found in '{dir_path}'. "
            "Expected output from graph_merge step."
        )
    ...
    for fp in scene_files:
        ...
        # Keep relations as they are from graph_merge
        # Make sure they are correctly formatted as expected by the generation
        raw_relations = scene.get("relations", [])
        normalized_relations = []
        # Find segment IDs for all boxes in the graph merge scene
        box_seg_ids = {}
        for b in scene.get("boxes", []):
            box_seg_ids[b.get("index", b.get("id"))] = b.get("seg_ids", [])
            
        for rel in raw_relations:
            subj_idx = rel.get("subject_index")
            obj_idx = rel.get("object_index")
            
            # Get the segments inside these subjects and objects
            subj_segs = box_seg_ids.get(subj_idx, rel.get("subject_seg_ids", []))
            obj_segs = box_seg_ids.get(obj_idx, rel.get("object_seg_ids", []))
            
            # If a subject or object was NOT matched to segments, fallback to a single list with the index itself 
            # (though graph_merge usually provides seg_ids)
            if not subj_segs: subj_segs = [subj_idx]
            if not obj_segs: obj_segs = [obj_idx]
            
            # For each subject segment and each object segment, create a 1-to-1 relation.
            # This turns "A -> [B, C, D]" into "A -> B", "A -> C", "A -> D".
            for s_seg in subj_segs:
                for o_seg in obj_segs:
                    # Do not create self-loops
                    if s_seg == o_seg: continue
                    
                    normalized_relations.append({
                        # Use the segment ID as the new index so it matches the attribute objects which are indexed by seg_id
                        "subject_index": s_seg,
                        "object_index": o_seg,
    ...
    print(f"Loaded {len(items)} scenes with relations from {dir_path}")
    total_rels = sum(len(s.get("relations", [])) for _, s in items)
    print(f"Total relations loaded from graph_merge: {total_rels}")
    
    return {img_id: scene for img_id, scene in items}


def load_attributes_scenes(path: str):
    """Load scenes from attributes.json or attributes.jsonl. Group by image_id."""
    data = []
    ...
    for row in data:
        ...
        scenes[img_id].append(row)
        
    items = []
    for img_id, rows in scenes.items():
        boxes = []
        for i, r in enumerate(rows):
            attrs = r.get("attributes", {})
            if "visual_reasoning" in attrs:
                attrs = {k: v for k, v in attrs.items() if k != "visual_reasoning"}
                
            boxes.append({
                "index": r.get("index", r.get("seg_id", i)),
                "label": r.get("category", r.get("label", "Unknown")),
                "bbox_xyxy": r.get("bbox", [0, 0, 0, 0]),
                "attributes": attrs,
                "seg_ids": [r.get("seg_id")] if "seg_id" in r else [],
                "id": r.get("seg_id", i)
            })
    ...
    print(f"Loaded {len(items)} scenes with attributes from {path}")
    return items


def parse_args():
    p = argparse.ArgumentParser()
    p.add_argument("--attributes_json", required=True, help="Path to attributes.json or attributes.jsonl")
    p.add_argument("--graph_merge_dir", required=False, help="Path to directory containing scene-graph_*.json from graph_merge step")
    ...
    p.add_argument("--multiple_choice_method", default=False, action="store_true", help="Use multiple choice (A,B,C,D) for relationship questions")
...
def main():
    ...
    print(f"Using attributes from: {args.attributes_json}")
    raw_items = load_attributes_scenes(args.attributes_json)
    
    if args.graph_merge_dir:
        print(f"Merging with relations from graph_merge directory: {args.graph_merge_dir}")
        relations_dict = load_graph_merge_scenes(args.graph_merge_dir)
        
        merged_items = []
        for img_id, scene in raw_items:
            if img_id in relations_dict:
                scene["relations"] = relations_dict[img_id].get("relations", [])
            merged_items.append((img_id, scene))
