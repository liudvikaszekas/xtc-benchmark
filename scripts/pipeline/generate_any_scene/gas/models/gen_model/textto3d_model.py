...
class Prolificdreamer(GenModelInstance):
    def __init__(self, ckpt: list = ["t3d/configs/prolificdreamer.yaml", "t3d/configs/prolificdreamer-geometry.yaml","t3d/configs/prolificdreamer-texture.yaml"], precision: torch.dtype = torch.float16, device: torch.device = torch.device("cuda"), geometry_refine = False, texturing = False):
        self.config_f = ckpt
        self.device = device
...
class Magic3D(GenModelInstance):
    def __init__(self, ckpt: str = ["t3d/configs/magic3d-coarse-if.yaml", "t3d/configs/magic3d-refine-sd.yaml"], precision: torch.dtype = torch.float16, device: torch.device = torch.device("cuda"), refine = False):
        self.config_f = ckpt
        self.device = device
        self.refine = refine
    def gen(self, prompt):
...
class LatentNeRF(GenModelInstance):
    def __init__(self, ckpt: list = ["t3d/configs/latentnerf.yaml", "t3d/configs/latentnerf-refine.yaml"], precision: torch.dtype = torch.float16, device: torch.device = torch.device("cuda"), refine = False):
        self.config_f = ckpt
        self.device = device
        self.refine = refine
    def gen(self, prompt):
