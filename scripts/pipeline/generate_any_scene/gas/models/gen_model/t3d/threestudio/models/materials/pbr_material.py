...
class PBRMaterial(BaseMaterial):
    ...
    class Config(BaseMaterial.Config):
        ...
        environment_texture: str = f"{current_dir}/../../../load/lights/mud_road_puresky_1k.hdr"
    ...
    def configure(self) -> None:
        ...
        FG_LUT = torch.from_numpy(
            np.fromfile(f"{current_dir}/../../../load/lights/bsdf_256_256.bin", dtype=np.float32).reshape(
                1, 256, 256, 2    
            )
            # np.fromfile("t3d/load/lights/bsdf_256_256.bin", dtype=np.float32).reshape(
            #     1, 256, 256, 2
            # )
        )
