...
class Text2ImageEvalMetric():
    def __init__(
        ...
        if selected_metrics is None:
            selected_metrics = list(metrics_dict.keys())  # use all the supported matrics
    ...
    def eval_with_metrics(self, output: Image.Image, gen_data: dict):
        ...
        for metric_name in self.metrics:
            if isinstance(self.metrics[metric_name], str):
                ...
                results[metric_name] = current_metric.compute(image = image, gen_data = gen_data)
            else:
...
class ClipScore(Metric):
    def __init__(self, model_name_or_path='openai:ViT-L-14-336', device="cuda"):
        ...
        import t2v_metrics
        self.clipscore = t2v_metrics.CLIPScore(model='openai:ViT-L-14-336', device=self.device)
