...
imageqa_models = {
    ...
    "Phi-3-vision-128k-instruct"  : ("Phi3Vision", "microsoft/Phi-3-vision-128k-instruct"),
...
class ImageQAModel(QAModel):
	def __init__(
		...
		elif isinstance(torch_device, int):
			if torch_device == -1:
...
class Phi3Vision(QAModelInstance):
    def __init__(self, ckpt="microsoft/Phi-3-vision-128k-instruct", torch_device=torch.device("cuda"), model_precision=torch.bfloat16):
        ...
        self.model = AutoModelForCausalLM.from_pretrained(
            ...
            trust_remote_code=True,
            torch_dtype=model_precision,
            _attn_implementation='eager' # disable flash-attn for compatible on clusters
        ).to(torch_device).eval() 
    ...
    def qa(self, image, prompt):
        ...
        inputs = self.processor(
            self.processor.tokenizer.apply_chat_template(messages, tokenize=False, add_generation_prompt=True),
            [image],
            return_tensors="pt"
        ).to(self.device)
